...
| 2  | `audit-context.json` must be written to `$BRAIN_ROOT/_Docs/<slug>/Reports/` (not to the source repo). The skill must never write audit artifacts into the project's source tree. |
...
| 6  | When `--resume` is used, the skill must re-read the most recent report from `_Docs/<slug>/Reports/` and continue from the first unresolved finding. It must not re-scan the entire codebase or re-apply already-fixed findings. |
